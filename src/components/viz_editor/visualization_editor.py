        ...
        dmc.Stack(
          [
            dmc.Card(
              dmc.Grid(
                [
                  dmc.GridCol(scenarios_select(value=init_scenarios)),
                  dmc.GridCol(models_select(value=init_models)),
                  dmc.GridCol(location_select(value=init_location)),
                  dmc.GridCol(target_select(value=init_target)),
                  dmc.GridCol(age_group_select(value=init_age_group)),
                  dmc.GridCol(uncertainty_select(value=init_uncertainty)),
                  dmc.GridCol(ensemble_select(value=init_ensemble)),
                ],
                gutter=8,
              ),
              variant='soft',
            ...
            dmc.Card(
              dmc.Grid(
                [dmc.GridCol(zoom_control(value=init_zoom))],
                gutter=8,
              ),
              variant='soft',
            ),
            dmc.Card(
              dmc.Grid(
                [dmc.GridCol(annotations_input(value=init_annotations))],
                gutter=8,
              ),
              variant='soft',
            ),
